...
class ZGTTest {
  ...
  @DisplayName(
      "pushes 1 if second top is greater than 0, 0 otherwise, ie a -> c where c=1 if a>0 and c=0 if !a>0.")
  ...
  void worksOkWithNumbers(Object parameter, Object expectedResult, String parameterClassName) {
    // GIVEN
    var zgt = PrimitiveFactory.ZGT();
    var zgtAddr = nextInt();
    var ip = anInstructionPointer().with(zgtAddr).build();
    var nip = aNextInstructionPointer().with(ip).plus(1).build();
    var state1 =
        aMachineState()
            .withInstrcutionPointer(ip)
            .withNextInstructionPointer(nip)
            .withMemory(aMemory().with(zgtAddr, zgt).build())
  ...
  static class NumberArgumentProvider implements ArgumentsProvider {
    ...
    public Stream<? extends Arguments> provideArguments(ExtensionContext context) throws Exception {
      return Stream.of(
          ...
          Arguments.of(
              RandomUtils.nextBigDecimalExcluding(0).abs(), 1, BigDecimal.class.getCanonicalName()),
          ...
          Arguments.of(
              RandomUtils.nextBigIntegerExcluding(0).abs(), 1, BigInteger.class.getCanonicalName()),
