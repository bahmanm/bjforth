...
class PRINTTest {
  ...
  void worksOk() {
    // GIVEN
    var PRINTaddr = BootstrapUtils.getPrimitiveAddress("PRINT");
    var n = RandomUtils.insecure().randomInt();
    var actualState =
        aMachineState()
            .withInstrcutionPointer(anInstructionPointer().with(PRINTaddr).build())
    ...
    assertThat(outputStream.toByteArray()).containsExactly(String.valueOf(n).getBytes());
  }
}
