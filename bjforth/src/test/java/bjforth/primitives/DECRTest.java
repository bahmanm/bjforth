...
class DECRTest {
  ...
  @DisplayName("decrements top of stack by 1, ie a -> a-1.")
  ...
  void worksOkWithNumber(Object parameter, Object expectedResult, String parameterClassName) {
    // GIVEN
    var decr = PrimitiveFactory.DECR();
    var decrAddr = nextInt();
    var ip = anInstructionPointer().with(decrAddr).build();
    var nip = aNextInstructionPointer().with(ip).plus(1).build();
    var state1 =
        aMachineState()
            .withInstrcutionPointer(ip)
            .withNextInstructionPointer(nip)
            .withMemory(aMemory().with(decrAddr, decr).build())
  ...
  static class NumberArgumentProvider implements ArgumentsProvider {
    ...
    private Arguments ofBigDecimal() {
      ...
      var nMinusOne = n.subtract(BigDecimal.ONE);
      return Arguments.of(n, nMinusOne, n.getClass().toString());
    }

    private Arguments ofBigInteger() {
      var n = RandomUtils.nextBigInteger();
      var nMinusOne = n.subtract(BigInteger.ONE);
      return Arguments.of(n, nMinusOne, n.getClass().toString());
    }

    private Arguments ofByte() {
      var n = RandomUtils.nextByte();
      var nMinusOne = (byte) (n - 1);
      return Arguments.of(n, nMinusOne, n.getClass().toString());
    }
